...
__inline __int64 _mm_get_pi64x(const __m64 i) {
	union {
		__int64 i;
		__m64 v;
	} u;
...
void Skein_512_Process_Block_mmx(uint64_t* T, uint64_t* X, const uint8_t *blkPtr, size_t blkCnt, size_t byteCntAdd)
{
	...
	ts[0] = _mm_set_pi64x(T[0]);
	ts[1] = _mm_set_pi64x(T[1]);
	do {
		ts[0] = _mm_add_si64(ts[0], _mm_set_pi32(0, byteCntAdd));

		z1 = SKEIN_KS_PARITY;
		ks[0] = _mm_set_pi64x(X[0]);
		z1 = _mm_xor_si64(z1, ks[0]);
		ks[1] = _mm_set_pi64x(X[1]);
		z1 = _mm_xor_si64(z1, ks[1]);
		ks[2] = _mm_set_pi64x(X[2]);
		z1 = _mm_xor_si64(z1, ks[2]);
		ks[3] = _mm_set_pi64x(X[3]);
		z1 = _mm_xor_si64(z1, ks[3]);
		ks[4] = _mm_set_pi64x(X[4]);
		z1 = _mm_xor_si64(z1, ks[4]);
		ks[5] = _mm_set_pi64x(X[5]);
		z1 = _mm_xor_si64(z1, ks[5]);
		ks[6] = _mm_set_pi64x(X[6]);
		z1 = _mm_xor_si64(z1, ks[6]);
		ks[7] = _mm_set_pi64x(X[7]);
...
#define R512_8_rounds(R) \
		...
		X[0] = _mm_get_pi64x(_mm_xor_si64(X0, w[0]));
		X[1] = _mm_get_pi64x(_mm_xor_si64(X1, w[1]));
		X[2] = _mm_get_pi64x(_mm_xor_si64(X2, w[2]));
		X[3] = _mm_get_pi64x(_mm_xor_si64(X3, w[3]));
		X[4] = _mm_get_pi64x(_mm_xor_si64(X4, w[4]));
		X[5] = _mm_get_pi64x(_mm_xor_si64(X5, w[5]));
		X[6] = _mm_get_pi64x(_mm_xor_si64(X6, w[6]));
		X[7] = _mm_get_pi64x(_mm_xor_si64(X7, w[7]));

		ts[1] = _mm_and_si64(ts[1], _mm_set_pi32(~(((uint32_t)64) << 24), ~0));
	} while (--blkCnt);
	T[0] = _mm_get_pi64x(ts[0]);
	T[1] = _mm_get_pi64x(ts[1]);
	_mm_empty();
}

void Skein_256_Process_Block_mmx(uint64_t* T, uint64_t* X, const uint8_t *blkPtr, size_t blkCnt, size_t byteCntAdd)
{
	__m64  kw[8];                         /* key schedule words : chaining vars + tweak */
	__m64  X0, X1, X2, X3;        /* local copy of vars, for speed */
	__m64  w[4];                          /* local copy of input block */
	__m64  z1;
	__m64  z3;
	...
	do {
		...
		X0 = _mm_add_si64(X0, ks[0]);
		X1 = _mm_add_si64(X1, _mm_add_si64(ks[1], ts[0]));
		X2 = _mm_add_si64(X2, _mm_add_si64(ks[2], ts[1]));
		X3 = _mm_add_si64(X3, ks[3]);
...
#define I256(R)                                                     \
    X0 = _mm_add_si64(X0,ks[((R)+1) % 5]);   /* inject the key schedule value */  \
    X1 = _mm_add_si64(X1,_mm_add_si64(ks[((R)+2) % 5],ts[((R)+1) % 3]));          \
    X2 = _mm_add_si64(X2,_mm_add_si64(ks[((R)+3) % 5],ts[((R)+2) % 3]));          \
    X3 = _mm_add_si64(X3,_mm_add_si64(ks[((R)+4) % 5],_mm_set_pi32(0,(R)+1)));     \
